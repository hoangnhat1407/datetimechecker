...
Collection **"DateTimeChecker API"** (file `Topic 2-API testing/DateTimeChecker API.postman_collection.json`):
- 2 requests: `POST {{baseUrl}}/api/datetime/check` and `GET {{baseUrl}}/api/datetime/check?day={{day}}&month={{month}}&year={{year}}`
